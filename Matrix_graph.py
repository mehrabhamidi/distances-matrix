...
# graph = {1: [[2, 1], [3, 9], [4, 4]], 2: [[3, 2], [5, 3], [1, 1]], 3: [[2, 2], [1, 9], [4, 2]],
#          4: [[3, 2], [1, 4], [5, 1]], 5: [[4, 1], [2, 3]]}
graph = {}
for i in range(dim):
    graph[i] = []
...
# graph_distance = dijkstra(graph, 1, 5, [0 for _ in range(6)], 0, [99999 for _ in range(6)])
...
for clause in sorted_list:
    ...
    if key < dim:
        ...
        i = int(key / dim)
    j = key - i * dim
    graph_distance = dijkstra(graph, i, j, [0 for _ in range(dim)], 0, [99999 for _ in range(dim)])
    # print(i, j)
